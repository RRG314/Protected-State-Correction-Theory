...
| --- | --- | --- | --- |
| Structural Discovery Studio | constrained-observation branch, restricted PVRT spine, fiber-separation exactness, collapse-modulus classification, `κ(η)/2` lower bound, exact-regime upper envelope, same-rank insufficiency, restricted-linear design logic, minimal augmentation count, periodic support thresholds, control finite-history versus observer split, validated repair demos | active branch / proved restricted-linear core / family-level threshold results / design-and-diagnosis surface | `src/ocp/recoverability.py`, `src/ocp/design.py`, `src/ocp/structural_discovery.py`, `docs/structural-discovery/*.md`, `docs/theory/advanced-directions/constrained-observation-*.md`, `docs/theory/advanced-directions/pvrt-*.md`, `docs/theorem-candidates/pvrt-theorem-spine.md`, `docs/workbench/lib/compute.js` |
...
| CFD Projection Lab | periodic incompressible projection fit, bounded finite-mode Hodge exactness, divergence-only bounded no-go, bounded-domain limitation | proved / restricted exact / no-go | `src/ocp/cfd.py`, `docs/cfd/*.md`, `docs/workbench/lib/compute.js` |
...
| No-Go Explorer | OCP-N1, OCP-N5, OCP-N7, divergence-only no-go, bounded-domain transplant rejection | proved / rejected-bridge layer | `docs/impossibility-results/*.md`, `docs/physics/bounded-domain-projection-limits.md`, `docs/workbench/lib/compute.js` |
