...
This document explains how the active branches fit together and why their scope boundaries are intentional.

The repository has one foundation and several strengthening lanes. The foundation is the exact/asymptotic/no-go core. Strengthening lanes develop constrained-observation recoverability, anti-classifier structure, augmentation design, invariants, and geometry-sensitive PDE behavior. Tooling lanes expose these results through reproducible interfaces.
...
The program is strongest when read as one coherent structure with strict labels: core theorem/no-go backbone first, branch-limited strengthening second, tooling and diagnostics third.
