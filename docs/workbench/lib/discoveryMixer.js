...
const EPS = 1e-10;
const DIAGONAL_EIGENVALUES = [0.95, 0.8, 0.65, 0.5];
...
function diagonalRecordMatrix(eigenvalues, sensorWeights, horizon) {
  return Array.from({ length: horizon }, (_, t) => sensorWeights.map((value, index) => value * (eigenvalues[index] ** t)));
}
...
function solveLeastSquares(matrix, vector) {
  ...
  const mt = Array.from({ length: cols }, (_, j) => matrix.map((row) => row[j]));
  const normal = Array.from({ length: cols }, (_, i) => Array.from({ length: cols }, (_, j) => mt[i].reduce((sum, value, row) => sum + value * matrix[row][j], 0)));
  const rhs = mt.map((row) => row.reduce((sum, value, index) => sum + value * vector[index], 0));
